...
class FirstFrame():
    ...
    def singleCapture(self):
        ...
        if(fileSeleted == True):
            ...
            time.sleep(1)
            if (bustStatus == True):
                ImageGrab.grab().save(working_dir + "//screen Shot/Step %d-%d.png" %(number,bustNumber), "PNG")
                bustStatus = False
            else:
                ImageGrab.grab().save(working_dir + "//screen Shot/Step %d.png" %number, "PNG")
        ...
        if(fileSeleted == True):
            if (bustStatus == True):
                ...
                if (bustNumber == 0):
                    number -=1
                    print
                    os.remove(working_dir + "//screen Shot/%d.png" %(number))
                    myText.set("Step Number : %d" %(number))
                else:

                    print "Bust nmber : ",bustNumber
                    os.remove(working_dir + "//screen Shot/%d-%d.png" %(number,bustNumber))
                    myText.set("Step Number : %d.%d" %(number,bustNumber))
